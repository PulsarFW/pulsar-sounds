...
### Shared sound playback — world/ambient sound effects and UI sound effects, played through `plsr.Sounds`
...
Registers `plsr.Sounds` on both sides. Splits into two lanes — `sounds.lua` (general playback) and `uisounds.lua` (interaction/UI feedback sounds) — with a small static NUI page as the actual audio playback surface.
